...
def _create_digest_baseline_table(connection: sqlite3.Connection) -> None:
    connection.execute(
        "CREATE TABLE daily_digest_baselines ("
        "calendar_date TEXT PRIMARY KEY, "
...
def _validate_schema_columns(
    ...
    if validate_digest_baselines and (
        _table_columns(connection, "daily_digest_baselines")
        != _DIGEST_BASELINE_COLUMNS
    ):
        raise SqlitePersistenceError(
