...
class _MaintenanceCommandListener:
    """Read, serialize and report explicit retention commands."""
    ...
    def __init__(
        ...
        publish_preview: Callable[[datetime], bool],
    ) -> None:
        ...
        if not isinstance(processor, MaintenanceCommandProcessor):
            raise TypeError("processor must be a MaintenanceCommandProcessor")
        if not callable(publish_preview):
            raise TypeError("publish_preview must be callable")
